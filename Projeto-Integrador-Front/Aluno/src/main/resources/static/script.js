const form = document.getElementById('alunoImagemForm');
const alunoImagemList = document.getElementById('alunoImagemList');
...
form.addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch('http://localhost:8080/alunoImagem', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(alunoImagem)
        });
